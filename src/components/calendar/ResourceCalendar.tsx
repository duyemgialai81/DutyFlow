...
import { Avatar, AvatarStack, EmptyState, SHIFT_META, Tip } from "../ui";
...
function ScheduleChip({ a, s, draggable, onOpen, onDragStart }: {
  ...
  const meta = SHIFT_META[s.shiftCode];
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="tnum block truncate font-mono text-[10.5px] font-semibold leading-tight text-gray-700">
          {s.startTime}–{s.endTime}
        </span>
        <span className="block truncate text-[10px] leading-tight text-gray-400">{s.location}</span>
      </span>
      {locked ? (
        ...
        <Clock3 className="h-3 w-3 shrink-0 text-blue-400" />
      )}
      {draggable && <GripVertical className="h-3 w-3 shrink-0 text-gray-200 opacity-0 transition-opacity group-hover/chip:opacity-100" />}
    </button>
  );
}
...
function DropCell({ cellKey, date, employee, children, onMoveRequest, disabled, highlight, setHighlight, onQuickCreate, isAdmin, empty }: {
  ...
  return (
    <div
      ...
      className={cx(
        "relative min-h-[62px] space-y-1 border-b border-r border-edgesoft p-1.5 transition-colors duration-150",
        active && "bg-blue-50 ring-2 ring-inset ring-brand-500/60",
  ...
  return (
    <div className={cx(
      ...
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate text-[11px] text-gray-400">{e.departmentName ?? "—"}</span>
      </span>
    </div>
  );
});
...
function LegendRow() {
  return (
    <div className="sticky left-0 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-edgesoft bg-gray-50/70 px-4 py-2.5 text-[11px] font-medium text-gray-500">
      <span className="flex items-center gap-1.5"><CheckCircle2 className="h-3.5 w-3.5 text-green-500" /> Đã xác nhận</span>
      <span className="flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5 text-blue-400" /> Chờ xác nhận</span>
      <span className="flex items-center gap-1.5"><AlertTriangle className="h-3.5 w-3.5 text-red-500" /> Thiếu nhân sự</span>
      <span className="flex items-center gap-1.5"><Lock className="h-3.5 w-3.5 text-gray-400" /> Đã khóa</span>
      <span className="ml-auto hidden text-gray-400 sm:block">Kéo thả ca giữa các nhân viên để đổi người trực</span>
    </div>
  );
}
