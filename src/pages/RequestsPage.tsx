...
import { Avatar, Button, ConfirmDialog, DayOffStatusBadge, EmptyState, Field, inputCls, Segmented, Skeleton } from "../components/ui";
...
export function RequestsPage() {
  ...
  const { push } = useToast();
  const [tab, setTab] = useState<"swap" | "off">("swap");
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<OffValues>({
      ...
      <div className="anim-rise flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[13.5px] text-sub">
            {isAdmin ? `${pendingSwaps.length} yêu cầu đổi ca chờ duyệt` : "Gửi yêu cầu đổi ca hoặc đăng ký nghỉ"}
          </p>
        </div>
        <Segmented
        ...
        <div className="space-y-3">
          {!isAdmin && (
            <div className="anim-rise flex items-start gap-3 rounded-[14px] border border-blue-100 bg-blue-50/50 px-4 py-3.5 text-[13px] leading-relaxed text-gray-600">
              ...
              <span>Yêu cầu đổi ca được gửi từ trang <b className="text-ink">Ca của tôi</b> — chọn ca và bấm “Yêu cầu đổi ca”. Quản trị viên sẽ xem xét tại đây.</span>
            </div>
          )}
          {swaps.isLoading ? (
                  ...
                  <div key={s.id} className="anim-rise flex flex-wrap items-center gap-3 rounded-[14px] border border-edge bg-surface px-4 py-3.5 shadow-[var(--shadow-card)] transition-all hover:shadow-[var(--shadow-lift)]">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[13.5px] font-bold text-ink">
                        {s.employeeName}
                        <span className="font-medium text-gray-400"> · {s.departmentName}</span>
                      </p>
                      <p className="mt-0.5 text-[12.5px] text-gray-600">
                        Muốn đổi ca <b>{s.shiftName.toLowerCase()}</b> ngày <b>{fmtDateLong(s.scheduleDate)}</b> ({s.startTime}–{s.endTime}, {s.location})
                      </p>
                      <p className="mt-1 text-[12px] italic text-gray-400">“{s.reason}” · {timeAgo(s.createdAt)}</p>
                    </div>
      ...
      {tab === "off" && (
        <div className="grid gap-5 lg:grid-cols-[300px_1fr]">
          <div className="anim-rise h-fit rounded-[14px] border border-edge bg-surface p-5 shadow-[var(--shadow-card)] lg:sticky lg:top-20">
            ...
            <p className="mb-4 text-[12px] leading-snug text-gray-400">Nhân viên nghỉ (đã duyệt hoặc chờ duyệt) sẽ bị loại khỏi phân ca tự động ngày đó.</p>
          ...
          <div className="space-y-2.5">
            {offs.isLoading ? (
              ...
              <div className="anim-rise rounded-[14px] border border-edge bg-surface shadow-[var(--shadow-card)]">
                <EmptyState icon={<CalendarOff className="h-7 w-7" />} title="Chưa có đăng ký nghỉ" message={isAdmin ? "Nhân viên chưa gửi đăng ký nào." : "Bạn chưa đăng ký nghỉ ngày nào."} />
              </div>
            ) : (
              <div className="stagger space-y-2.5">
                {(offs.data ?? []).slice().sort((a, b) => a.date.localeCompare(b.date)).map((d) => (
                  <div key={d.id} className="flex flex-wrap items-center gap-3 rounded-[14px] border border-edge bg-surface px-4 py-3 shadow-[var(--shadow-card)] transition-all hover:shadow-[var(--shadow-lift)]">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-[13px] font-bold text-ink">{isAdmin ? `${d.employeeName} · ` : ""}{fmtDate(d.date)}</p>
