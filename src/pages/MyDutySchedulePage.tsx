...
import { ArrowRightLeft, CalendarCheck2, Check, Clock3, MapPin, ShieldCheck, X } from "lucide-react";
...
export function MyDutySchedulePage() {
  ...
  if (isLoading) {
    return <div className="mx-auto max-w-3xl space-y-3">{[0, 1, 2].map((i) => <Skeleton key={i} className="h-32 w-full" />)}</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      {past.length > 0 && (
        <section>
          <h3 className="mb-2.5 text-[11.5px] font-bold uppercase tracking-[0.14em] text-gray-400">Gần đây</h3>
          <div className="space-y-2.5">
            {past.map((i) => (
              <div key={i.assignmentId} className="flex flex-wrap items-center gap-3 rounded-[14px] border border-edge bg-surface px-4 py-3 opacity-75 shadow-[var(--shadow-card)]">
                ...
                <span className="text-[12.5px] text-gray-500">{i.schedule.startTime}–{i.schedule.endTime} · {i.schedule.location}</span>
