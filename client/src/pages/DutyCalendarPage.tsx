...
function FilterPopover({ filters, setFilters, departments, shifts, onClose, count }: {
  ...
  return (
    <>
      ...
      <div className="anim-pop absolute right-0 top-full z-40 mt-2 w-72 rounded-xl border border-edge bg-surface p-4 shadow-[var(--shadow-pop)]">
        ...
        <label className="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-sub">Phòng ban</label>
        ...
        <label className="mb-1.5 mt-3 block text-[11.5px] font-bold uppercase tracking-wide text-sub">Ca trực</label>
        <Select value={draft.shiftId ?? ""} onChange={(e) => setDraft((d) => ({ ...d, shiftId: e.target.value ? Number(e.target.value) : undefined }))}>
          <option value="">Tất cả</option>
          {shifts.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
        </Select>
        <label className="mb-1.5 mt-3 block text-[11.5px] font-bold uppercase tracking-wide text-sub">Trạng thái</label>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="anim-rise rounded-[14px] border border-edge bg-surface shadow-[var(--shadow-card)]">
        ...
        {/* desktop calendar */}
        <div className="hidden md:block overflow-hidden rounded-b-[14px]">
          <ResourceCalendar
            schedules={schedules}
        ...
        <div className="md:hidden">
          <div className="flex gap-2 overflow-x-auto border-b border-edgesoft px-3 py-3">
            {mobileDays.map((d) => {
              ...
              return (
                <button
                  ...
                  className={cx(
                    "touch-target flex min-h-[56px] min-w-[56px] flex-col items-center justify-center rounded-[10px] border px-2.5 py-1.5 transition-all",
                    active ? "border-brand-600 bg-brand-600 text-white shadow-[0_2px_8px_rgb(37_99_235/0.35)]" : "border-edge bg-surface text-gray-700 hover:border-gray-300",
                    d === todayISO() && !active && "border-blue-300 bg-blue-50/70 font-semibold",
                  )}
                >
                  <span className={cx("text-[10px] font-bold uppercase", active ? "text-blue-100" : "text-sub")}>
                    {["CN", "T2", "T3", "T4", "T5", "T6", "T7"][parseISO(d).getDay()]}
                  </span>
                  <span className="tnum text-[15px] font-bold leading-tight">{Number(d.slice(8, 10))}</span>
                  <span className={cx("text-[9.5px] font-semibold", active ? "text-blue-100" : count > 0 ? "text-brand-600" : "text-sub")}>
                    {count > 0 ? `${count} ca` : "—"}
                  </span>
                </button>
              );
          ...
          <div className="space-y-2.5 p-3">
            {mobileSchedules.length === 0 && (
              <div className="py-10 text-center">
                <CalendarDays className="mx-auto mb-2 h-6 w-6 text-sub" />
                <p className="text-[13px] font-semibold text-ink">Không có ca trực</p>
                <p className="text-[12px] text-sub">Ngày {fmtDate(mobileDate)} chưa có lịch.</p>
              </div>
            )}
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12.5px] font-bold uppercase text-sub">{s.shiftName}</span>
                    {missing
                      ? <span className="ml-auto rounded-md bg-red-50 px-2 py-0.5 text-[11px] font-bold text-red-600 border border-red-200">Thiếu {s.requiredPeople - activeAssign.length}</span>
                      : <span className="ml-auto rounded-md bg-green-50 px-2 py-0.5 text-[11px] font-bold text-green-700 border border-green-200">Đủ</span>}
                  </div>
                  <div className="mt-2 flex items-center gap-2">
                    <AvatarStack names={activeAssign.map((a) => a.employeeName)} max={5} size="sm" />
                    <span className="truncate text-[12px] text-sub">{s.location}</span>
                  </div>
                </button>
              );
            })}
      ...
      {canManage && <AutoAssignModal open={autoOpen} onClose={() => setAutoOpen(false)} preset={autoPreset} />}
