...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const adminNav: NavItem[] = [
    { to: "/overview", label: "Tổng quan", icon: <LayoutDashboard className="h-5 w-5" /> },
    { to: "/duty/calendar", label: "Lịch trực", icon: <CalendarDays className="h-5 w-5" /> },
    { to: "/users", label: "Nhân viên", icon: <Users className="h-5 w-5" /> },
    { to: "/shifts", label: "Ca trực", icon: <Clock3 className="h-5 w-5" /> },
    { to: "/requests", label: "Đổi ca", icon: <Repeat className="h-5 w-5" /> },
    { to: "/notifications", label: "Thông báo", icon: <Bell className="h-5 w-5" /> },
    { to: "/reports", label: "Báo cáo", icon: <BarChart3 className="h-5 w-5" /> },
    { to: "/settings", label: "Cài đặt", icon: <Settings className="h-5 w-5" /> },
  ];
  const leaderNav: NavItem[] = [
    ...
    { to: "/notifications", label: "Thông báo", icon: <Bell className="h-5 w-5" /> },
    { to: "/reports", label: "Báo cáo", icon: <BarChart3 className="h-5 w-5" /> },
    { to: "/settings", label: "Cài đặt Zalo", icon: <Settings className="h-5 w-5" /> },
  ];
  const employeeNav: NavItem[] = [
    { to: "/overview", label: "Tổng quan", icon: <LayoutDashboard className="h-5 w-5" /> },
    { to: "/duty/calendar", label: "Lịch trực", icon: <CalendarDays className="h-5 w-5" /> },
    { to: "/my-duty", label: "Ca của tôi", icon: <CalendarCheck2 className="h-5 w-5" /> },
    { to: "/requests", label: "Đổi ca", icon: <Repeat className="h-5 w-5" /> },
    { to: "/notifications", label: "Thông báo", icon: <Bell className="h-5 w-5" /> },
    { to: "/settings", label: "Cài đặt", icon: <Settings className="h-5 w-5" /> },
  ];
  const nav = isAdmin ? adminNav : isLeader ? leaderNav : employeeNav;
  const meta = PAGE_META[location.pathname] ?? PAGE_META["/overview"];

  const mobileNavItems = isAdmin || isLeader
    ? [
  ...
  const SidebarInner = ({ compact, onNavigate }: { compact: boolean; onNavigate?: () => void }) => (
    <div className="flex h-full flex-col">
      ...
      <nav className="flex-1 space-y-2 overflow-y-auto px-3.5 py-5" aria-label="Điều hướng chính">
        {!compact && <p className="px-3.5 pb-3 pt-1 text-[11px] font-bold uppercase tracking-[0.14em] text-gray-400">Quản lý</p>}
        {nav.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            ...
            className={({ isActive }) =>
              cx(
                "group flex h-11 items-center gap-3.5 rounded-xl px-3.5 text-[14px] font-medium transition-all duration-150",
                compact && "justify-center px-0",
                isActive ? "bg-blue-50 font-semibold text-brand-700 shadow-sm" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900",
              )
            }
          >
            {({ isActive }) => (
      ...
      <div className="border-t border-edgesoft p-3.5">
        <button
          onClick={persistCollapse}
          className="hidden h-10 w-full items-center justify-center gap-2 rounded-xl text-[13px] font-semibold text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-800 lg:flex"
          aria-label={collapsed ? "Mở rộng sidebar" : "Thu gọn sidebar"}
        >
          {collapsed ? <ChevronsRight className="h-4.5 w-4.5" /> : <><ChevronsLeft className="h-4.5 w-4.5" /> Thu gọn</>}
        </button>
      </div>
    </div>
  );
          ...
          <div className="flex h-16 items-center gap-3 px-4 sm:px-6">
            ...
            <div className="ml-auto flex items-center gap-2 sm:gap-3">
              ...
              <div className="relative" ref={menuRef}>
                ...
                >
                  <Avatar name={user?.name || "Người dùng"} size="sm" />
                  <span className="hidden text-left sm:block">
                    <span className="block max-w-[130px] truncate text-[13px] font-semibold leading-tight text-ink">{user?.name || "Người dùng"}</span>
                    <span className="block text-[11px] leading-tight text-gray-400">{isAdmin ? "Quản trị viên" : isLeader ? "Tổ trưởng" : "Nhân viên"}</span>
                  </span>
                ...
                {menuOpen && (
                  <div className="anim-pop absolute right-0 z-50 mt-2 w-64 overflow-hidden rounded-xl border border-edge bg-surface shadow-[var(--shadow-pop)]" role="menu">
                    <div className="border-b border-edgesoft px-4 py-3">
                      <p className="text-[13px] font-bold text-ink">{user?.name || "Người dùng"}</p>
                    ...
                    <div className="px-2 py-2">
                      <button
                        ...
                        className="flex w-full items-center gap-2.5 rounded-lg px-2 py-2 text-left text-red-600 transition-colors hover:bg-red-50"
                      >
                        <span className="min-w-0 flex-1">
                          <span className="block truncate text-[13px] font-semibold">Đăng xuất</span>
                        </span>
                      </button>
                    </div>
                  </div>
